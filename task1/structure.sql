...
CREATE TABLE `feed_category` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_category_feed` (`category_id`,`feed_id`),
  KEY `feed_id` (`feed_id`),
  KEY `idx_category_id` (`category_id`), -- Need for Inner Join and Selects
  CONSTRAINT `feed_category_ibfk_1` FOREIGN KEY (`category_id`) REFERENCES `category` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `feed_category_ibfk_2` FOREIGN KEY (`feed_id`) REFERENCES `feed` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8;
...
CREATE TABLE `feed_user_like` (
  ...
  `feed_id` int(11) unsigned NOT NULL,
  `islike` tinyint(1) DEFAULT NULL COMMENT '1- like, 2-unlike, null - no user desicion',
  PRIMARY KEY (`id`),
  UNIQUE KEY `user_id` (`user_id`,`feed_id`), -- uniq for insert 
  KEY `idx_user_id` (`user_id`), -- Need for Inner Join and Selects
  KEY `feed_id` (`feed_id`), -- Need for Inner Join and Selects
  CONSTRAINT `feed_user_like_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `feed_user_like_ibfk_2` FOREIGN KEY (`feed_id`) REFERENCES `feed` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8;
...
Where feed_id IN (1,2) and IFNULL(feed_user_like.islike,0) > 0
